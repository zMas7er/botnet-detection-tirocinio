...
def plot_tuning_results(cv_results, param_name, model_name):
    ...
    plt.title(f"Tuning Parametrico: {model_name} ({param_name})")
    ...
    plt.savefig(os.path.join(plots_dir, f"tuning_{model_name}_{clean_param}.png"), dpi=300)
    plt.close()
...
def run_statistical_tuning():
    ...
    for name, (base_model, params) in models.items():
        ...
        # Generiamo il grafico per il primo parametro testato
    ...
    for name, pipeline in best_estimators.items():
        ...
        for i in range(ITERATIONS):
            ...
            # Scaler e SMOTE vanno fittati SOLO su X_train
            scaler = StandardScaler()
            ...
            # Estraiamo il modello dalla pipeline trovata dal GridSearch
            model = pipeline.named_steps["model"]
            model.fit(X_train_res, y_train_res)
            
            # Soglia su Validation
            ...
            # Salviamo log e grafici completi solo per l'ultima iterazione
            if i == ITERATIONS - 1:
                exp_name = f"tuning_stats_{DATASET_NAME}_{name}"
