...
print("--- FASE 5 (v2): MITIGAZIONE - SOLO FEATURE ROBUSTE (STATISTICA 15 RUN) ---")
ctu_path = os.path.join(base_dir := os.path.dirname(os.path.abspath(__file__)), "data/raw/ctu13")
...
robust_features = ['Dur', 'SrcBytes', 'TotBytes', 'TotPkts']
feature_cols = [c for c in robust_features if c in df_clean.columns]
missing = [c for c in robust_features if c not in df_clean.columns]
if missing:
    print(f"⚠️ Attenzione: feature attese ma non trovate nel dataset: {missing}")
print(f"Feature usate (solo quelle robuste secondo SHAP): {feature_cols}")
...
print(f"\n=== RISULTATI STATISTICI MITIGAZIONE 2 (Solo Feature Robuste) ===")
final_results = {}
...
save_results({"XGBoost_OnlyRobustFeatures_Statistical": final_results}, f"results/mitigation_v2_robust_only_statistical_{TARGET_ZERO_DAY}.json")
print(f"\nRisultati statistici salvati in results/mitigation_v2_robust_only_statistical_{TARGET_ZERO_DAY}.json")