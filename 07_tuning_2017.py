...
def run_statistical_tuning():
    ...
    raw_dir = os.path.join(base_dir, "data", "raw", "cic_ids2017")
    df_raw = load_cic(raw_dir, source_name=DATASET_NAME)
    
    # Rimozione colonne duplicate o buggate note del 2017
    bugged_cols = [c for c in df_raw.columns if c.endswith('.1') or c == 'FwdHeader Length']
    if bugged_cols:
        print(f"Rimozione colonne duplicate/buggate dal 2017: {bugged_cols}")
        df_raw = df_raw.drop(columns=bugged_cols)
