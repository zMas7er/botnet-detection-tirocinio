...
def run_mitigation_pruning():
    print(f"--- 06: MITIGAZIONE 1 (FEATURE PRUNING) - TARGET: {TARGET_ZERO_DAY} ---")
    
    raw_dir = os.path.join(base_dir, "data", "raw", "ctu13")
    df_clean = clean_numeric(load_ctu13(raw_dir, fraction_benign=0.1))
    
    # Rimuoviamo le feature volumetriche (come da Capitolo 6.2)
    leaky_features = ['Dur', 'TotBytes', 'SrcBytes', 'TotPkts']
    feature_cols = [c for c in get_feature_columns(df_clean) if c not in leaky_features]
    print(f"Feature rimosse: {leaky_features}")
    print(f"Feature rimanenti: {len(feature_cols)}")
    ...
    for i in range(ITERATIONS):
        ...
        # Loggiamo l'ultima iterazione per avere i grafici di riferimento
        if i == ITERATIONS - 1:
            exp_name = f"mitigation_pruning_{TARGET_ZERO_DAY}"
    ...
    print("\n=== RISULTATI STATISTICI (MITIGAZIONE 1: PRUNING) ===")
    for key, values in accumulated_metrics.items():
        print(f"  {key}: {np.mean(values):.4f} ± {np.std(values):.4f}")

if __name__ == "__main__":
    run_mitigation_pruning()